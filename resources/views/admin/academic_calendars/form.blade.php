    ...
    <select name="university_id" id="university_id" class="form-select @error('university_id') is-invalid @enderror">
        ...
        @foreach ($universities as $univ)
            <option value="{{ $univ->id }}"
                {{ old('university_id', $academic_calendar->university_id ?? '') == $univ->id ? 'selected' : '' }}>
                {{ $univ->name }}
            </option>
        @endforeach
    </select>
...
<div class="row mb-3">
    <div class="col-md-6">
        ...
        <input type="text" name="starts_on" id="starts_on"
            ...
            value="{{ old('starts_on', isset($academic_calendar) ? $academic_calendar->starts_on->format('Y-m-d') : '') }}"
            required>
        @error('starts_on')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
    ...
    <div class="col-md-6">
        ...
        <input type="text" name="ends_on" id="ends_on"
            ...
            value="{{ old('ends_on', isset($academic_calendar) ? $academic_calendar->ends_on->format('Y-m-d') : '') }}"
            required>
        @error('ends_on')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
