    ...
    @forelse($batches as $batch)
      <tr>
        ...
        <td class="small text-muted">
          {{ $batch->university_id ?? '—' }}
          @if($batch->college_id) / {{ $batch->college_id }} @endif
          @if($batch->major_id)   / {{ $batch->major_id }}   @endif
        </td>
        ...
        <td class="text-center">
          <a href="{{ route('admin.activation_code_batches.show', $batch) }}" class="btn btn-sm btn-outline-secondary">
            عرض
          </a>
          <a href="{{ route('admin.activation_code_batches.edit', $batch) }}" class="btn btn-sm btn-outline-primary">
            تعديل
          </a>
          <a href="{{ route('admin.activation_code_batches.export', $batch) }}" class="btn btn-sm btn-outline-success">
            تصدير
          </a>
          <form action="{{ route('admin.activation_code_batches.destroy', $batch) }}" method="POST" class="d-inline">
            @csrf @method('DELETE')
            <button class="btn btn-sm btn-outline-danger" onclick="return confirm('حذف الدفعة؟ سيتم إبقاء الأكواد لكن بدون ربط الدفعة.')">حذف</button>
          </form>
        </td>
      </tr>
    @empty
