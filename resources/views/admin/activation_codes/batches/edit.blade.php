...
<body class="p-3">
<div class="container">
  <h3 class="mb-3">تعديل دفعة #{{ $batch->id }}</h3>
  ...
  <form method="post" action="{{ route('admin.activation-code-batches.update',$batch->id) }}" class="row g-3">
    @csrf @method('PUT')
    <div class="col-md-6">
      <label class="form-label">الاسم *</label>
      <input type="text" name="name" class="form-control" value="{{ old('name',$batch->name) }}" required maxlength="150">
    </div>
    <div class="col-md-6">
      <label class="form-label">الخطة (plan_id) *</label>
      <input type="number" name="plan_id" class="form-control" value="{{ old('plan_id',$batch->plan_id) }}" required>
    </div>

    <div class="col-md-4">
      <label class="form-label">university_id</label>
      <input type="number" name="university_id" class="form-control" value="{{ old('university_id',$batch->university_id) }}">
    </div>
    <div class="col-md-4">
      <label class="form-label">college_id</label>
      <input type="number" name="college_id" class="form-control" value="{{ old('college_id',$batch->college_id) }}">
    </div>
    <div class="col-md-4">
      <label class="form-label">major_id</label>
      <input type="number" name="major_id" class="form-control" value="{{ old('major_id',$batch->major_id) }}">
    </div>
    ...
    <div class="col-md-3">
      ...
      <input type="number" name="duration_days" class="form-control" value="{{ old('duration_days',$batch->duration_days) }}" min="1" required>
    </div>
    <div class="col-md-3">
      ...
      <select name="start_policy" class="form-select" required>
        @foreach(['on_redeem','fixed_start'] as $p)
          <option value="{{ $p }}" @selected(old('start_policy',$batch->start_policy)===$p)>{{ $p }}</option>
        @endforeach
      </select>
    </div>
    ...
    <div class="col-md-4">
      ...
      <input type="date" name="starts_on" class="form-control" value="{{ old('starts_on',$batch->starts_on) }}">
    </div>
    <div class="col-md-4">
    ...
    <div class="col-md-4">
      ...
      <input type="number" name="code_length" class="form-control" value="{{ old('code_length',$batch->code_length) }}" min="6" max="64" required>
    </div>
    <div class="col-md-4">
      <label class="form-label">created_by_admin_id</label>
      <input type="number" name="created_by_admin_id" class="form-control" value="{{ old('created_by_admin_id',$batch->created_by_admin_id) }}">
    </div>
    ...
    <div class="col-12 d-flex gap-2">
      ...
      <a href="{{ route('admin.activation-code-batches.show',$batch->id) }}" class="btn btn-secondary">رجوع</a>
    </div>
  </form>
</div>
</body>
