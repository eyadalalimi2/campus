...
<div class="row g-3">
  <div class="col-md-4">
    ...
    <select name="batch_id" id="batch_id" class="form-select">
      ...
      @foreach(\App\Models\ActivationCodeBatch::orderBy('id','desc')->get(['id','name','code_length','code_prefix']) as $b)
        <option value="{{ $b->id }}" data-length="{{ $b->code_length }}" data-prefix="{{ $b->code_prefix }}" @selected(old('batch_id',$code->batch_id ?? '')==$b->id)>{{ $b->name }}</option>
      @endforeach
    </select>
  </div>
  <div class="col-md-4">
  ...
  <div class="col-md-4">
    ...
    <input type="text" name="code" class="form-control" value="{{ old('code',$code->code ?? '') }}" placeholder="اتركه فارغًا لتوليد تلقائي">
  </div>

  <div class="col-12"><hr><strong>النطاق (اختياري)</strong></div>
  <div class="col-md-4">
  ...
  <div class="col-md-4">
    ...
    <select name="college_id" id="college_select" class="form-select">
      ...
      @foreach(\App\Models\College::orderBy('name')->get() as $c)
        <option value="{{ $c->id }}" data-university="{{ $c->university_id }}" @selected(old('college_id',$code->college_id ?? '')==$c->id)>{{ $c->name }}</option>
      @endforeach
    </select>
  </div>
  <div class="col-md-4">
    ...
    <select name="major_id" id="major_select" class="form-select">
      ...
      @foreach(\App\Models\Major::with('college')->orderBy('name')->get() as $m)
        <option value="{{ $m->id }}" data-college="{{ $m->college_id }}" @selected(old('major_id',$code->major_id ?? '')==$m->id)>{{ $m->name }}</option>
      @endforeach
    </select>
  </div>
  ...
  <div class="col-md-3">
    ...
    <input type="number" name="duration_days" class="form-control" min="1" max="1825" value="{{ old('duration_days',$code->duration_days ?? 365) }}" required>
  </div>
  <div class="col-md-3">
  ...
  <div class="col-md-3 start-on">
    ...
    <input type="date" name="starts_on" class="form-control" value="{{ old('starts_on', optional($code->starts_on ?? null)->format('Y-m-d')) }}">
  </div>
  <div class="col-md-3">
    <label class="form-label">صالح من</label>
    <input type="datetime-local" name="valid_from" class="form-control" value="{{ old('valid_from', optional($code->valid_from ?? null)->format('Y-m-d\TH:i')) }}">
  </div>
  <div class="col-md-3">
    <label class="form-label">صالح حتى</label>
    <input type="datetime-local" name="valid_until" class="form-control" value="{{ old('valid_until', optional($code->valid_until ?? null)->format('Y-m-d\TH:i')) }}">
  </div>
  <div class="col-md-3">
    <label class="form-label">حد الاستخدام</label>
    <input type="number" name="max_redemptions" class="form-control" min="1" max="1000" value="{{ old('max_redemptions',$code->max_redemptions ?? 1) }}" required>
  </div>
  <div class="col-md-3">
