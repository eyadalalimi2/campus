@extends('admin.layouts.app')
@section('title','جهاز جديد')

@section('content')
<div class="d-flex align-items-center justify-content-between mb-3">
  <h1 class="h4 m-0">جهاز جديد</h1>
  <a href="{{ route('admin.med_devices.index') }}" class="btn btn-light"><i class="bi bi-arrow-right"></i> رجوع</a>
</div>

<form action="{{ route('admin.med_devices.store') }}" method="POST" enctype="multipart/form-data" class="card p-3">
  @csrf
  @include('admin.med_devices.form', ['device' => null, 'selected' => old('subject_ids',[])])
  <div class="mt-3"><button class="btn btn-success"><i class="bi bi-check2-circle"></i> حفظ</button></div>
</form>
@endsection
