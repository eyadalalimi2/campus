                ...
                <div class="col-md-4">
                    ...
                    <select name="system_id" class="form-select">
                        ...
                        @foreach ($systems as $s)
                            <option value="{{ $s->id }}"
                                {{ (string) request('system_id') === (string) $s->id ? 'selected' : '' }}>{{ $s->name_ar }}
                            </option>
                        @endforeach
                    </select>
                </div>
                    ...
                    <select name="subject_id" class="form-select">
                        ...
                        @foreach ($subjects as $subj)
                            <option value="{{ $subj->id }}"
                                {{ (string) request('subject_id') === (string) $subj->id ? 'selected' : '' }}>
                ...
                <tbody>
                    @forelse($items as $x)
                        <tr>
                            ...
                            <td>{{ $x->level }}</td>
                            <td>
                                <a href="{{ route('medical.system-subjects.edit', $x) }}" class="btn btn-sm btn-warning"><i
                                        class="bi bi-pencil"></i> تعديل</a>
                                <form action="{{ route('medical.system-subjects.destroy', $x) }}" method="post"
                                    class="d-inline">@csrf @method('DELETE')
                                    <button class="btn btn-sm btn-danger" onclick="return confirm('حذف؟')"><i
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="text-center">لا توجد روابط</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
